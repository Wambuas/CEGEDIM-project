...
```{r}
objective1_dataSpain = read.csv("obj1_spn.csv")

dput(names(objective1_dataSpain))
...
```{r}
glimpse(objective1_dataSpain)
```

```{r}
n_distinct(objective1_dataSpain$person_id)
```


```{r}
objective1_dataSpain[objective1_dataSpain==""] <- NA #set blanks to NA
```


```{r}
colMeans(is.na(objective1_dataSpain))
```
...
objective1_dataSpain = filter(objective1_dataSpain,gender_code %in% c("F","M"))
...
```{r}
objective1_dataSpain = objective1_dataSpain %>% dplyr::mutate(
  ...
  birth_date2 = as.Date(birth_date,format = "%Y-%m-%d")
...
```{r}
range(objective1_dataSpain$death_date,na.rm = T)

table(objective1_dataSpain$death_date)
...
```{r}
denominator_2022 = objective1_dataSpain
```
...
denominator_2022 = objective1_dataSpain %>% dplyr::mutate(
...
##### 3. Remove those with no GP visit in 2021 
...
denominator_2022_adults = denominator_2022_adults %>% dplyr::mutate(
  ...
  age_cats = cut(age_2022, breaks=c(-Inf, 34, 44, 54, 64,150), labels=c("18 to 34","35 to 44","45 to 54","55 to 64",">65")))
...
```{r}
denominator_2023 = objective1_dataSpain
```

##### 1. Remove those with CRS diagnosis before 2023 

```{r}

denominator_2023 = objective1_dataSpain %>% dplyr::mutate(
  crs_before_2021 = ifelse(!is.na(N_CRS_allDiag_before_2021),1,0),
  crs_in_2021 = ifelse(!is.na(CRS_allDiag_2021),1,0),
...
##### 2. Remove those with nasal polyps before end of 2023 
...
##### 4. Remove those with no GP visit in 2023 
...
##### 5. Remove those who died before or on start of 2023 if any 
...
##### 5. Remove those with last contact date with physician before 2023 
...
##### Keep events to those with atleast 2 CRS diagnosis in 2023

```{r}
denominator_2023$crs_outcome_2023_v2 = ifelse(denominator_2023$crs_outcome_2023==1 & !is.na(denominator_2023$CRS_allDiag_2023) & denominator_2023$CRS_allDiag_2023 >1,1,0)

k =subset(denominator_2023,denominator_2023$crs_outcome_2023_v2==1)
k = k %>% dplyr::select(person_id,CRS_date,crs_outcome_2023_v2,CRS_allDiag_2023)

table(denominator_2023$crs_outcome_2023_v2)

```


```{r}
denominator_2023$crs_outcome_2023 = denominator_2023$crs_outcome_2023_v2
table(denominator_2023$crs_outcome_2023)
...
denominator_2023_adults = denominator_2023_adults %>% dplyr::mutate(
  ...
  age_cats = cut(age_2023, breaks=c(-Inf, 34, 44, 54, 64,150), labels=c("18 to 34","35 to 44","45 to 54","55 to 64",">65")))
...
```{r}
denominator_2024 = objective1_dataSpain
```

##### 1. Remove those with CRS diagnosis before 2024 

```{r}

denominator_2024 = objective1_dataSpain %>% dplyr::mutate(
...
##### 2. Remove those with nasal polyps before end of 2024 
...
##### 3. Remove those with no GP visit in 2023 
...
##### 4. Remove those with no GP visit in 2024 
...
##### 5. Remove those who died before or on start of 2024 if any 
...
##### 5. Remove those with last contact date with physician before 2024 
...
##### Keep events to those with atleast 2 CRS diagnosis in 2024

```{r}
denominator_2024$crs_outcome_2024_v2 = ifelse(denominator_2024$crs_outcome_2024==1 & !is.na(denominator_2024$CRS_allDiag_2024) & denominator_2024$CRS_allDiag_2024 >1,1,0)

k =subset(denominator_2024,denominator_2024$crs_outcome_2024_v2==1)
k = k %>% dplyr::select(person_id,CRS_date,crs_outcome_2024_v2,CRS_allDiag_2024)

table(denominator_2024$crs_outcome_2024_v2)

```


```{r}
denominator_2024$crs_outcome_2024 = denominator_2024$crs_outcome_2024_v2
table(denominator_2024$crs_outcome_2024)
...
denominator_2024_adults = denominator_2024_adults %>% dplyr::mutate(
  ...
  age_cats = cut(age_2024, breaks=c(-Inf, 34, 44, 54, 64,150), labels=c("18 to 34","35 to 44","45 to 54","55 to 64",">65")))
...
write.csv(combined,"CombinedSpain_Crude.csv",row.names = F)
...
crude_incidenceSpain = rbind(paeds1,adults1)

write.csv(crude_incidenceSpain,"Crude_Incidence_NoCIsSpain.csv")
...
crude_incidence2Spain = rbind(paeds1,adults1)

write.csv(crude_incidence2Spain,"Crude_Incidence_WithCIsSpain.csv")
...
combinedSpain_data = rbind(denom_year_2022,denom_year_2023,denom_year_2024)
...
aggregated_data = combinedSpain_data %>% dplyr::group_by(age_cats,gender_code,Year,Dataset) %>% dplyr::summarise(
...
standard_population = read.csv("/rds/projects/w/wangzz-thin-tardyferon/rhinoNasal/Data analysis/Eurostat/demo_pjan_page_linear_2_0_spn.csv")[,c(8,9,11,16)]
...
```{r}
std_incidenceSpain = rbind(paeds_combined1,adult_combined)
write.csv(std_incidenceSpain,"Standardized_Incidence_NoCIsSpain.csv")
...
```{r}
std_incidenceSpain = rbind(paeds_combined,adult_combined)
write.csv(std_incidenceSpain,"Standardized_Incidence_WithCIsSpain.csv")
...
```{r}
crude_inc = read.csv("Crude_Incidence_NoCIsSpain.csv")
crude_inc$Metric = "Crude"
standardized_inc = read.csv("Standardized_Incidence_NoCIsSpain.csv")
...
write.csv(final_clean,"All_results_Spain.csv",row.names = F)
