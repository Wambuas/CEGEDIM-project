...
```{r}
objective1_data_UK = read.csv("obj1_uk.csv")

dput(names(objective1_data_UK))
...
```{r}
glimpse(objective1_data_UK)
```

```{r}
n_distinct(objective1_data_UK$person_id)
```


```{r}
objective1_data_UK[objective1_data_UK==""] <- NA #set blanks to NA
```


```{r}
colMeans(is.na(objective1_data_UK))
```

### Variables needed for follow-up calculation

```{r}
objective1_data_UK = objective1_data_UK %>% dplyr::mutate(
...
```{r}
range(objective1_data_UK$death_date,na.rm = T)

table(objective1_data_UK$death_date)
...
```{r}
denominator_2022 = objective1_data_UK
```
...
denominator_2022 = objective1_data_UK %>% dplyr::mutate(
...
```{r}
denominator_2023 = objective1_data_UK
```
...
denominator_2023 = objective1_data_UK %>% dplyr::mutate(
  crs_before_2021 = ifelse(!is.na(N_CRS_allDiag_before_2021),1,0),
  crs_in_2021 = ifelse(!is.na(CRS_allDiag_2021),1,0),
...
```{r}
denominator_2024 = objective1_data_UK
```
...
denominator_2024 = objective1_data_UK %>% dplyr::mutate(
...
write.csv(combined,"Combined_UK_Crude.csv",row.names = F)
...
crude_incidence_UK = rbind(paeds1,adults1)

write.csv(crude_incidence_UK,"Crude_Incidence_NoCIs_UK.csv")
...
crude_incidence2_UK = rbind(paeds1,adults1)

write.csv(crude_incidence2_UK,"Crude_Incidence_WithCIs_UK.csv")
...
combined_uk_data = rbind(denom_year_2022,denom_year_2023,denom_year_2024)
...
aggregated_data = combined_uk_data %>% dplyr::group_by(age_cats,gender_code,Year,Dataset) %>% dplyr::summarise(
...
standard_population = read.csv("/rds/projects/w/wangzz-thin-tardyferon/rhinoNasal/Data analysis/Eurostat/227097786699562_uk_Census_2021.csv")

standard_population$age1 = 0:90

standard_population$age_cats = cut(standard_population$age1, breaks=c(-Inf, 5, 11, 17,34,44,54,64,100), labels=c("0 to 5","6 to 11","12 to 17","18 to 34","35 to 44","45 to 54","55 to 64",">65"))
...
standard_population = standard_population %>% dplyr::select(
  ...
  age_cats,Female,Male
  
)

standard_population = reshape2::melt(standard_population,id="age_cats")
...
aggregated_data$gender_code = ifelse(aggregated_data$gender_code=="F","Female","Male")
...
```{r}
std_incidence_UK = rbind(paeds_combined1,adult_combined)
write.csv(std_incidence_UK,"Standardized_Incidence_NoCIs_UK.csv")
...
```{r}
std_incidence_UK = rbind(paeds_combined,adult_combined)
write.csv(std_incidence_UK,"Standardized_Incidence_WithCIs_UK.csv")
...
```{r}
crude_inc = read.csv("Crude_Incidence_NoCIs_UK.csv")
crude_inc$Metric = "Crude"
standardized_inc = read.csv("Standardized_Incidence_NoCIs_UK.csv")
...
write.csv(final_clean,"All_results_UK.csv",row.names = F)
